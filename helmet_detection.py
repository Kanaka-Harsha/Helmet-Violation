...
# 3. Open Video
# RTSP Stream (Live) - Replace with your IP Camera URL
# VIDEO_PATH = "rtsp://admin:password@192.168.1.64:554/Streaming/Channels/101" 
...
while True:
    ...
    for bike_box in bikes:
        ...
        # Check for HELMET (if no violation found yet, or just to be sure)
        # If a helmet is found, we might want to prioritize that as "SAFE" even if no_helmet was triggered (false positive no_helmet)
        # But for now, let's say if ANY no_helmet is found, it's a violation unless overruled.
        # Simple logic: If helmet found -> Safe. If no helmet found -> Violation check.
        
        for h_box in helmets:
            if is_overlap(h_box, bike_box):
                has_helmet = True
                break

        if has_helmet:
            violation_detected = False # Safe
